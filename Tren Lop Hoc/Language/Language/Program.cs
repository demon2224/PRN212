...
Dong 3";
            ...
            int age = 22;
            Console.WriteLine("Ten toi la " + name + " va toi " + age + " !"); //cach thong dung
            Console.WriteLine($"Ten toi la {name} va toi {age} !"); //cach rut gon lai (.Net)
            ...
            // Kiem tra xem da qua sinh nhat chua (chi chinh xac neu can)
            DateTime currentDate = DateTime.Now;
            Console.WriteLine("Nhap thang sinh (1-12):");
            int birthMonth = Convert.ToInt32(Console.ReadLine());
            Console.WriteLine("Nhap ngay sinh (1-31):");
            int birthDay = Convert.ToInt32(Console.ReadLine());
            
            DateTime birthDate = new DateTime(birthYear, birthMonth, birthDay);
            int exactAge = currentDate.Year - birthDate.Year;
            if (currentDate < birthDate.AddYears(exactAge))
            {
                exactAge--; // Chua qua sinh nhat nam nay
            }
            
            Console.WriteLine($"=== KET QUA ===");
            Console.WriteLine($"Ten: {name1}");
            Console.WriteLine($"Nam sinh: {birthYear}");
            Console.WriteLine($"Ngay sinh: {birthDate:dd/MM/yyyy}");
            Console.WriteLine($"Tuoi (don gian): {calculatedAge}");
            Console.WriteLine($"Tuoi (chinh xac): {exactAge}");
            Console.WriteLine($"So ngay da song: {(currentDate - birthDate).Days} ngay");
            
            // Them thong tin thu vi
            string zodiac = GetZodiacSign(birthMonth, birthDay);
            string generation = GetGeneration(birthYear);
            Console.WriteLine($"Cung hoang dao: {zodiac}");
            Console.WriteLine($"The he: {generation}");
        ...
        private static string GetZodiacSign(int month, int day)
        {
            ...
            {
                (3, >= 21) or (4, <= 19) => "Bach Duong (Aries)",
                (4, >= 20) or (5, <= 20) => "Kim Nguu (Taurus)", 
                (5, >= 21) or (6, <= 20) => "Song Tu (Gemini)",
                (6, >= 21) or (7, <= 22) => "Cu Giai (Cancer)",
                (7, >= 23) or (8, <= 22) => "Su Tu (Leo)",
                (8, >= 23) or (9, <= 22) => "Xu Nu (Virgo)",
                (9, >= 23) or (10, <= 22) => "Thien Binh (Libra)",
                (10, >= 23) or (11, <= 21) => "Bo Cap (Scorpio)",
                (11, >= 22) or (12, <= 21) => "Nhan Ma (Sagittarius)",
                (12, >= 22) or (1, <= 19) => "Ma Ket (Capricorn)",
                (1, >= 20) or (2, <= 18) => "Bao Binh (Aquarius)",
                (2, >= 19) or (3, <= 20) => "Song Ngu (Pisces)",
                _ => "Khong xac dinh"
            };
        }
        ...
        private static string GetGeneration(int birthYear)
        {
            return birthYear switch
            {
                ...
                >= 1965 and <= 1980 => "Gen X",
                >= 1946 and <= 1964 => "Baby Boomers",
                >= 1928 and <= 1945 => "Silent Generation",
                _ => "Khong xac dinh the he"
            };
        }
    }
}
